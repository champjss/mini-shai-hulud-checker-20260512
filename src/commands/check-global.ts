...
type PackageHit = { name: string; version: string; manifestPath: string };
type FileHit = { path: string; sha256: string };

export async function runCheckGlobal(): Promise<number> {
  const affected = loadAffected();
  const malicious = affected.maliciousFileHashes[ROUTER_INIT]?.sha256;
  if (!malicious) {
    throw new Error(
      `advisory/file-hashes.json is missing maliciousFileHashes["${ROUTER_INIT}"].sha256`,
    );
  }
  ...
  process.stdout.write(
    `Summary: ${totalPackageHits} compromised package(s), ${totalFileHits} compromised file(s) across ${locations.length} location(s).\n`,
  );
...
async function scanPackages(
  root: string,
  affectedMap: Record<string, string[]>,
): Promise<PackageHit[]> {
...
async function scanFiles(root: string, malicious: string): Promise<FileHit[]> {
  const hits: FileHit[] = [];
